{{-- Inbox product mockup — chat-first on mobile, full split on sm+ --}}
<x-marketing.browser-frame :light="true" url="connectpulse.cloud/inbox" class="w-full">
    <div class="flex min-h-[260px] sm:min-h-[320px]">
        <div class="hidden w-[38%] shrink-0 border-r border-slate-200 bg-white p-2 sm:block">
            @foreach([['Priya Sharma', 'Thanks, can we schedule...', true], ['Amit Patel', 'Report received', false], ['Neha Singh', 'Interested in demo', false]] as [$name, $preview, $active])
                <div class="mb-1 rounded-lg px-2 py-2.5 {{ $active ? 'bg-[#635bff]/10 ring-1 ring-[#635bff]/20' : '' }}">
                    <div class="flex items-center justify-between gap-1">
                        <p class="truncate text-[11px] font-semibold text-slate-900">{{ $name }}</p>
                        @if($active)<span class="h-2 w-2 shrink-0 rounded-full bg-[#635bff]"></span>@endif
                    </div>
        ...
        <div class="flex min-w-0 flex-1 flex-col bg-slate-50">
            <div class="border-b border-slate-200 bg-white px-3 py-2.5">
                ...
                <p class="truncate text-[10px] text-slate-500">+91 98765 43210 · Lead · Interested</p>
            </div>
            <div class="flex-1 space-y-2 p-3">
                <div class="ml-auto max-w-[90%] rounded-2xl rounded-br-sm bg-[#635bff] px-3 py-2 text-[11px] leading-snug text-white sm:max-w-[85%]">
                    Hi Priya, following up on your health package enquiry.
                </div>
                <div class="max-w-[90%] rounded-2xl rounded-bl-sm bg-white px-3 py-2 text-[11px] leading-snug text-slate-700 shadow-sm sm:max-w-[85%]">
                    Yes! Can we schedule a demo tomorrow?
                </div>
            </div>
            <div class="border-t border-slate-200 bg-white p-2">
                <div class="flex gap-2">
                    <div class="min-w-0 flex-1 truncate rounded-lg border border-slate-200 px-2 py-2 text-[10px] text-slate-400">Type a message...</div>
                    <span class="shrink-0 rounded-lg bg-emerald-600 px-3 py-2 text-[10px] font-semibold text-white">Send</span>
                </div>
            </div>
        </div>
        <div class="hidden w-[26%] shrink-0 border-l border-slate-200 bg-white p-3 lg:block">
