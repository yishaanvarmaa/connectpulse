@props(['lead', 'pendingFollowUp' => null])

<div class="cp-sticky-actions">
    <div class="cp-action-bar mx-auto max-w-md">
        <button
            type="button"
            data-whatsapp-open
            data-lead-name="{{ $lead->name }}"
            data-whatsapp-url="{{ route('org.crm.leads.whatsapp', $lead) }}"
            class="cp-action-bar-item cp-action-bar-item--accent"
        >
        ...
        <a href="tel:{{ $lead->phone }}" class="cp-action-bar-item">
        ...
        @if($pendingFollowUp)
            <button
                type="button"
                data-sheet-open="complete-{{ $pendingFollowUp->id }}"
                class="cp-action-bar-item"
            >
                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                <span>Log result</span>
            </button>
        @else
            <a href="#log-interaction" class="cp-action-bar-item">
